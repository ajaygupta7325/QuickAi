  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post(
        ...
        {
          headers: { Authorization: `Bearer ${await getToken()}` }  // ✅ fixed here
        }
      );
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {blogCategories.map((item) => (
            <span onClick={() => setSelectedCategories(item)}
              className={`rounded-full text-xs px-4 py-1 border cursor-pointer ${selectedCategories === item ? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
          ))}
        </div>
        <br />
    ...
    text-sm rounded-lg cursor-pointer'>
     ...
     border-gray-200 min-h-96 '>
      ...
      text-gray-400'>
                ...
                <p>Enter a topic and click "Generate BlogTitles" to get started</p>
              </div>
            </div>
          ) : (
