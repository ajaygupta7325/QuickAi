...
const GenerateImages = () => {
  const imageStyle = [
    "Realistic", "Anime style", "Cartoon style",
  ...
  try {
    ...
    if (data.success) {
      console.log("✅ secure_url:", data.secure_url); // for debugging
      setContent(data.secure_url); // ✅ this was the issue
    } else {
  ...
  return (
    ...
    items-start flex-wrap gap-4 text-slate-700'>
     ...
     rounded-lg border border-gray-200'>
        <div className='flex items-center gap-3'>
          <Sparkles className='w-6 text-[#04FF50]' />
        ...
        <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
          {imageStyle.map((item) => (
            <span onClick={() => setSelectedStyle(item)}
              className={`rounded-full text-xs px-4 py-1 border cursor-pointer ${selectedStyle === item ? 'bg-purple-50 text-green-700' : 'text-gray-500 border-gray-300'}`} key={item}>{item}</span>
          ))}
        </div>
    ...
    border-t-transparent animate-spin'></span> :
     ...
     border-gray-200 min-h-96 '>
        <div className='flex items-center gap-3'>
          <Edit className='w-5 h-5 text-[#04FF50]' />
        ...
        {!content ? (
          ...
          <div className='mt-3 min-h-[300px]'>
            {console.log("Image loaded:", content.slice(0, 50))}
            <img
              src={content}
              alt="AI generated"
